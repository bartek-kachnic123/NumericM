...
def falsi(n: int, G, a=0, b=1, epsilon=10e-10):
    ...
    if (n==3):
        ...
        G = F_derivate # change function to derivate

    x_steps = []
    xi = (a*G(b, n) - b*G(a, n)) / (G(b, n) - G(a, n)) # x1
    while np.abs(F(xi, n)) >= epsilon:

        x_steps.append(xi)

        if G(a, n) * G(xi, n) > 0:
            xi = (xi*G(a, n) - a*G(xi, n)) / (G(a,n) - G(xi,n))
        else:
            xi = (xi*G(b, n) - b*G(xi, n)) / (G(b,n) - G(xi,n))
